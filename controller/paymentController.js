        ...
        if (savedOrder) {
            ...
            const walletUpdate = await Wallet.updateOne(
                { user_id: new mongoose.Types.ObjectId(req.session.user_id) }, // Query by user ID
                {
                    ...
                    $push: { // Add new transaction to the array
                        transactions: {
            ...
            await Promise.all(cartProduct.Product.map(async (item) => {
...
const verifyPayment = async (req, res) => {
    ...
    try {
        ...
        console.log('Received payment details:', { payment_id, order_id, signature });
        ...
        const updateOrder = await Order.updateOne(
            { _id: savedOrder._id },
            { $set: { paymentMethod: razorpayPaymentMethod, paymentSource: paymentSource , payment:'success'} }
        );
        ...
        const updateOrder = await Order.updateOne(
            { _id: savedOrder._id },
            { $set: { paymentMethod: razorpayPaymentMethod, paymentSource: paymentSource , payment:'Failed'} }
        );
    }
};
