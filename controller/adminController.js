...
const adminLogindata = async (req, res) => {
  ...
  try {
    ...
    console.error(error);
    return res.status(500).render("admin/adminLogin", { toast: ["Server error"] });
  }
};
...
const addCategory = async (req, res) => {
  try {
    ...
    const is_exist= await Category.findOne({category_name:newCategory})
    if(is_exist){
      req.flash('info', `${newCategory} ❗The Category name already exist. Please enter a different name `);
    ...
    if (status) {
      req.flash('info', `${newCategory} was add succesfully ✅`);
      res.redirect("/category");
    } else {
      res.status(404).send("Category not found");
    }
